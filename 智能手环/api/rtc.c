...
u8 RTC_Init(void)
{
	...
	//		RTC_Delay_ms(5);
		...
		RTC->ISR&=~(1<<7);			//退出RTC初始化模式
		RTC->WPR=0xFF;				//使能RTC寄存器写保护  
		RTC_Set_Time(9,32,56,0);	//设置时间
		RTC_Set_Date(18,10,19,5);		//设置日期
...
{ 
	...
	RTC->WPR=0XFF;				//禁止修改RTC寄存器 　清除LINE22上的中断标志位  
...
//	timeStamp_t *time_p;
//	u8 ampm;
//	RTC_Get_Time(&time_p->hour,&time_p->minute,&time_p->second,&ampm);
//}
